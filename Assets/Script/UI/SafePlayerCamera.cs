...
namespace NaughtyCharacter
{
    public class SafePlayerCamera : MonoBehaviour
    {
        ...
        [Header("Lock On View Angle Fix / ロックオン見上げ対策")]
        [Tooltip("ON推奨。ロックオン時の上下角度だけ専用に補正します。下から見上げすぎる時に使います。")]
        [SerializeField] private bool useLockOnViewAngleFix = true;

        [Tooltip("ロックオン計算で使うプレイヤー側の視点高さです。0だと足元基準になりやすく、見上げが強くなります。1.0〜1.4推奨。")]
        [SerializeField] private float lockOnPlayerAimHeightOffset = 1.2f;

        [Tooltip("ロックオン計算で使う敵側の高さ補正です。下げたいならマイナス。ドラゴンを見上げすぎる場合は -0.5〜-1.5 推奨。")]
        [SerializeField] private float lockOnTargetAimHeightOffset = -0.8f;

        [Tooltip("ロックオン時のPitch補正です。プラスにすると見上げを抑えます。5〜12推奨。")]
        [SerializeField] private float lockOnPitchOffset = 8f;

        [Tooltip("ONならロックオン中だけ通常カメラとは別のPitch制限を使います。")]
        [SerializeField] private bool useSeparateLockOnPitchLimits = true;

        [Tooltip("ロックオン中の最小Pitchです。マイナスが大きいほど見上げます。見上げすぎるなら -8〜-15。")]
        [SerializeField] private float lockOnMinPitch = -10f;

        [Tooltip("ロックオン中の最大Pitchです。大きいほど見下ろせます。")]
